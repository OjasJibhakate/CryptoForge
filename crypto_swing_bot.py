# ===========================================================================
# CRYPTOFORGE :: INSTITUTIONAL SWING ENGINE (15-MINUTE TIMEFRAME)
# ===========================================================================
import json
import time
import pickle
...
CSV_FILE = "crypto_swing_trades.csv"     # 📝 ISOLATED SWING LEDGER

AI_CONFIDENCE_THRESHOLD = 0.70    
WHALE_WALL_USD = 2000000          
PROFIT_TARGET_PCT = 0.0200        # 🎯 2.0% Macro Target
STOP_LOSS_PCT = 0.0100            # 🛡️ 1.0% Wick Armor
...
def get_historical_klines(symbol, limit=80):
    try:
        # 🛡️ THE UPGRADE: Fetching 15m institutional data blocks instead of 1m noise
...
# =====================================================================
# 🧠 THE SWING PREDICTION CORE & SIGNAL RESOLVER
# =====================================================================
    ...
    try:
        ...
        if symbol in ACTIVE_POSITIONS:
            ...
            if high_price >= pos["target"]:
                print(f"🎉 [TARGET HIT] {symbol} locked 2.0% swing profit at ${pos['target']:,.4f}!")
                log_live_trade(symbol, "SELL", pos["target"], pos["prob"], pos["target"], pos["stop"], "WIN")
                del ACTIVE_POSITIONS[symbol]
            elif low_price <= pos["stop"]:
                print(f"🛡️ [STOP LOSS TRIGGERED] {symbol} cut at 1.0% structural floor ${pos['stop']:,.4f}!")
        ...
        new_row = pd.DataFrame([{
            ...
            "close_time": int(kline.get("T", kline["t"] + 899999)), # Patched for 15m close times
            "qav": float(kline["q"]),
        ...
        print(f"🔮 [15M AI SCAN] {symbol:<10} | Close: ${close_price:,.4f} | Prob: {probability*100:6.2f}%")
        
        # --- 3. ORDER PLACEMENT ---
        if probability >= AI_CONFIDENCE_THRESHOLD and symbol not in ACTIVE_POSITIONS:
            print("\n" + "="*50)
            print(f"🚨 [SWING BUY DETECTED] {symbol} at ${close_price:,.4f}")
...
def on_message(ws, message):
    ...
    try:
        ...
        if kline["x"]:
            print(f"\n⚡ [15M CANDLE CLOSE] {symbol} structural block locked in.")
...
def on_open(ws):
    print("\n📡 Connection Established. Streaming Live 15-Minute Institutional Data...")

if __name__ == "__main__":
    print("\n========================================================")
    print("🚀 BOOTING CRYPTOFORGE 15M SWING ENGINE...")
    print("========================================================")
    ...
    while True:
        ...
        # 🛡️ THE UPGRADE: Subscribing to the 15m WebSocket stream
        stream_params = "/".join([f"{s}@kline_15m" for s in WS_SYMBOLS])
